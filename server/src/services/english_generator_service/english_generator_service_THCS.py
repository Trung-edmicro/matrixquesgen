...
from .constants import (CLOZE_EXPLANATION_TEMPLATE, 
     ...
     CLOZE_WITH_TITLE_JSON_SCHEMA, CLOZE_JSON_SCHEMA,SENTENCE_COMPLETION_JSON_SCHEMA,SYNONYM_ANTONYM_JSON_SCHEMA,ERROR_IDENTIFICATION_JSON_SCHEMA,SENTENCE_TRANSFORMATION_JSON_SCHEMA,WORD_REORDERING_JSON_SCHEMA,PRONUNCIATION_STRESS_JSON_SCHEMA,DIALOGUE_COMPLETION_JSON_SCHEMA,LOGICAL_THINKING_JSON_SCHEMA,
       READING_COMPREHENSION_EXPLANATION_TEMPLATE, SILENT_PHASE_EXPLANATION_TEMPLATE)
from .english_constants_thcs import (COMPLETE_THE_SENTENCE_USING_THE_GIVEN_WORD_JSON_SCHEMA, 
                                     DIALOGUE_COMPLETION_THCS_JSON_SCHEMA, ERROR_IDENTIFICATION_THCS_JSON_SCHEMA, ESSAY_COMBINE_SENTENCES_JSON_SCHEMA, ESSAY_SENTENCE_REWRITING_THCS_JSON_SCHEMA, ESSAY_WORD_FORM_SENTENCE_COMPLETION_THCS_JSON_SCHEMA, ESSAY_WORD_ORDERING_THCS_JSON_SCHEMA,
                                     ESSAY_WORD_PROMPT_SENTENCE_COMPLETION_THCS_JSON_SCHEMA, ESSAY_WORD_ORDERING_THCS_JSON_SCHEMA, LOGICAL_THINKING_THCS_JSON_SCHEMA, OPENING_AND_ORDERING_SENTENCE_JSON_SCHEMA, ORDERING_AND_CLOSING_SENTENCE_JSON_SCHEMA, PROMPTS_ENGLISH_THCS_MAPPING,
                                     PRONUNCIATION_STRESS_THCS_JSON_SCHEMA, SENTENCE_BUILDING_JSON_SCHEMA, SENTENCE_COMPLETION_THPT_JSON_SCHEMA, SYNONYM_ANTONYM_THCS_JSON_SCHEMA, WORD_REORDERING_THCS_JSON_SCHEMA)
...
PROMPT_DIR_ENGLISH_THCS.mkdir(parents=True, exist_ok=True)
VOCABULARY_DIR.mkdir(parents=True, exist_ok=True)

DRIVE_FOLDER_THCS = "https://drive.google.com/drive/folders/1Nr5Uh9c6UBrChb0efwk3e9dUCZgJ32zf"
...
def sync_drive_prompts_to_local():
    ...
    try:
        folder_id = extract_folder_id(DRIVE_FOLDER_THCS)
        ...
        for file in files:
            ...
            local_path = PROMPT_DIR_ENGLISH_THCS / name
...
def load_prompt(filename: str) -> str:
    path = PROMPT_DIR_ENGLISH_THCS / filename
...
SEM = asyncio.Semaphore(5)   # có thể chỉnh 2–5 tùy quota
...
async def generate_exam_docx_thcs(blocks):
...
# )
    ...
    for block in blocks:
        ...
        try:
            prompt_template = load_prompt(PROMPTS_ENGLISH_THCS_MAPPING[q_type])
        except Exception:
            prompt_template = ""
        ...
        # ===============================
            ...
            output_rule_open_order = OPENING_AND_ORDERING_SENTENCE_JSON_SCHEMA.format(
                N_Q = n_q,
                START_NUM = q_count
            )

            formatted_opening_order_prompt = (prompt_template
                    .replace("{TOPIC_NAME}", safe_str(topic))
                    .replace("{TEXT_TYPE}", safe_str(text_type))
                    .replace("{CEFR_LEVEL}", safe_str(diff))
                    .replace("{VOCABULARY_LIST}", safe_str(vocabulary))
            ...
            ai_input_opening_order = (
                f"{formatted_opening_order_prompt}\n\n"
                f"Từ vựng tham khảo: {vocabulary}"
                f"Tài liệu tham khảo: {document_sample}"
                f"Đặc tả ma trận: {spec_item['spec']}\n"
                f"Mức độ: {spec_item['level']}\n"
            ...
            output_rule_order_closing = ORDERING_AND_CLOSING_SENTENCE_JSON_SCHEMA.format(
                N_Q = n_q,
                START_NUM = q_count
            )
            ...
            output_rule_sentence_completion_with_given_word = COMPLETE_THE_SENTENCE_USING_THE_GIVEN_WORD_JSON_SCHEMA.format(
                N_Q = n_q,
                START_NUM = q_count
            )

            formatted_logical_sentence_completion_with_given_word_prompt = (prompt_template
                    .replace("{TOPIC_NAME}", safe_str(topic))
                    .replace("{TEXT_TYPE}", safe_str(text_type))
                    .replace("{CEFR_LEVEL}", safe_str(diff))
                    .replace("{VOCABULARY_LIST}", safe_str(vocabulary))
            ...
            ai_input_sentence_completion_with_given_word = (
                f"{formatted_logical_sentence_completion_with_given_word_prompt}\n\n"
                f"Từ vựng tham khảo: {vocabulary}"
                f"Tài liệu tham khảo: {document_sample}"
                f"Đặc tả ma trận: {spec_item['spec']}\n"
                f"Mức độ: {spec_item['level']}\n"
                ...
                + "\n".join(specs_list)
                + "\n\n## OUTPUT FORMAT\n"
                + output_rule_sentence_completion_with_given_word
            )
            ...
            output_rule_sentence_rewriting = ESSAY_SENTENCE_REWRITING_THCS_JSON_SCHEMA.format(
                N_Q = n_q,
                START_NUM = q_count
            )
            ...
            output_rule_combine_sentence = ESSAY_COMBINE_SENTENCES_JSON_SCHEMA.format(
                N_Q = n_q,
                START_NUM = q_count
            )
            ...
            output_rule_ordering_word = ESSAY_WORD_ORDERING_THCS_JSON_SCHEMA.format(
                N_Q = n_q,
                START_NUM = q_count
            )
            ...
            output_essay_word_form_completion_thcs = ESSAY_WORD_FORM_SENTENCE_COMPLETION_THCS_JSON_SCHEMA.format(
                N_Q = n_q,
                START_NUM = q_count
            )
            ...
            output_essay_word_prompt_sentence_completion_thcs = ESSAY_WORD_PROMPT_SENTENCE_COMPLETION_THCS_JSON_SCHEMA.format(
                N_Q = n_q,
                START_NUM = q_count
            )
...
async def generate_english_flow(file):
    ...
    try:
        ...
        results = await generate_exam_docx_thcs(blocks, str(output_path))
...
async def generate_english_flow_thcs(file):
    session_id = str(uuid.uuid4())
