...
class QuestionGenerationService:
    ...
    def process_enriched_matrix(self, enriched_matrix_path: Path, question_types: List[str] = None) -> Optional[QuestionSet]:
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=min(10, len(generation_tasks))) as executor:
                # Submit all tasks
                future_to_task = {
    ...
    def _generate_question_task(self, task: Dict) -> List:
        """Generate questions for a single task (used in multi-threading)"""
        try:
            task_type = task['type']
            ...
            print(f"❌ Error in question generation task: {e}")
            return []
