...
class QuestionGenerator:
    ...
    def generate_tl_questions(self, 
                             ...
                             content: str = "") -> List[GeneratedQuestion]:
        """
        Sinh câu hỏi TL (Tự luận - Essay) cho một QuestionSpec
        ...
        Returns:
            List[GeneratedQuestion]: Danh sách câu hỏi TL đã sinh
        """        
        # Load template nếu cần
        if prompt_template_path and prompt_template_path != self.prompt_template:
            template = self._load_prompt_template(prompt_template_path)
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # Gọi AI với TL essay array schema - sử dụng fallback model nếu đã thử
                if tried_fallback:
                    print(f"🔄 Thử fallback model: {self.fallback_model}")
                    response = self.ai_client.generate_content_with_schema_with_model(
                        prompt=prompt_text,
                ...
                # Tạo GeneratedQuestion cho mỗi câu TL
                # TL questions có sample_answer thay vì correct_answer
                for i, question_data in enumerate(questions_data):
                    question_code = spec.question_codes[i] if i < len(spec.question_codes) else f"Q{i+1}"
                    
                    # Format explanation to include key_points and scoring_rubric
                    explanation_parts = []
                    if question_data.get("explanation"):
                        explanation_parts.append(f"Giải thích: {question_data['explanation']}")
                    
                    if question_data.get("key_points"):
                        key_points_text = "\n".join([
                            f"- {kp.get('point', '')} ({kp.get('weight', 0)} điểm): {kp.get('description', '')}"
                            for kp in question_data.get("key_points", [])
                        ])
                        explanation_parts.append(f"\nĐiểm kiến thức then chốt:\n{key_points_text}")
                    
                    if question_data.get("scoring_rubric"):
                        rubric = question_data.get("scoring_rubric", {})
                        rubric_text = f"\nThang điểm:\n" \
                                    f"- Xuất sắc (9-10): {rubric.get('excellent', '')}\n" \
                                    f"- Khá (7-8): {rubric.get('good', '')}\n" \
                                    f"- Trung bình (5-6): {rubric.get('average', '')}\n" \
                                    f"- Yếu (<5): {rubric.get('weak', '')}"
                        explanation_parts.append(rubric_text)
                    
                    full_explanation = "\n".join(explanation_parts) if explanation_parts else ""
                    
                    # TL không có options (A/B/C/D), có sample_answer dài
                    question = GeneratedQuestion(
                        question_code=question_code,
                        question_stem=question_data.get("question_stem", ""),
                        options={},  # Empty dict for TL
                        correct_answer=question_data.get("sample_answer", ""),  # Use sample_answer as correct_answer
                        level=question_data.get("level", spec.cognitive_level),
                ...
                if attempt < self.max_retries - 1:
                    ...
                    # Hết retry và hết fallback, tạo dummy questions
                    if self.verbose:
                        print(f"\n❌ Lỗi khi sinh câu hỏi TL sau {self.max_retries} lần thử")
                        if tried_fallback:
                            print(f"   (đã thử cả fallback model {self.fallback_model})")
                        print(f"   Error: {error_str}")
                        import traceback
