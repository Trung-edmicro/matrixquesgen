...
def generate_questions_task(
    ...
    For TOAN subjects: Pause after Phase 3 for template selection.
    For other subjects: Execute complete workflow.
    """
    ...
    try:
        ...
        if is_math:
            ...
            _log.info(f"[{session_id}] TOAN subject detected - will pause after Phase 3")
            ...
            processed_contents = orchestrator.execute_phase2_content_enrichment(
                matrix_output_path, matrix_metadata, lessons, drive_paths
            )
            ...
            # Save enriched matrix path to session for template selection
            session_data.update({
                "status": "awaiting_template_selection",
                "current_phase": "awaiting_template_selection",
                "progress": 75,
            ...
            _log.info(f"[{session_id}] Phase 3 complete. Enriched matrix: {mapping_result.enriched_matrix_path}")
            _log.info(f"[{session_id}] Waiting for template selection...")
            
            # Return early - frontend will show template selector
            # Phase 4 will be triggered by /math-template/continue-to-phase4 API
            return
            ...
            # Build question_sets list for processing
            question_sets = [question_set] if question_set else []
...
async def get_generation_progress(session_id: str):
    ...
    # Get status from root level first
    status = data.get("status", "unknown")
    
    # If status is completed AND metadata exists, use metadata values
    if status == "completed" and "metadata" in data:
    ...
    # Otherwise, use root level data (processing, awaiting_template_selection, etc.)
    return {
        "session_id": session_id,
        "status": status,
...
async def get_generation_result(session_id: str):
    ...
    if not questions_file.exists():
        ...
        if session_file.exists():
            ...
            elif status == 'awaiting_template_selection':
                raise HTTPException(
                    status_code=202,
